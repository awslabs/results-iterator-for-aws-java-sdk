...
public class V2HelperModule extends AbstractModule {
    ...
    protected void configure() {
        ...
        bind(StsClient.class).toProvider(new V2SafeProvider<>(StsClient::create));
        bind(S3Client.class).toProvider(new V2SafeProvider<>(S3Client::create));
        bind(AwsRegionProviderChain.class).toProvider(new V2SafeProvider<>(DefaultAwsRegionProviderChain::new));
        ...
        // NOTE: Using this pattern allows us to wrap the creation of these clients in some error checking code that can give the user information on what to do in the case of a failure
        bind(IamClient.class).toProvider(new V2SafeProvider<>(() -> IamClient.builder().region(Region.AWS_GLOBAL).build()));
